...
pub fn config_dir() -> PathBuf {
    ...
    std::env::current_exe().unwrap().parent().unwrap().join("config\\helix")
}
...
pub fn cache_dir() -> PathBuf {
    // TODO: allow env var override
    std::env::current_exe().unwrap().parent().unwrap().join("cache\\helix")
}
...
pub fn find_root_impl(root: Option<&str>, root_markers: &[String]) -> Vec<PathBuf> {
    let current_dir = PathBuf::from(std::env::current_exe().expect("unable to determine current directory").parent().unwrap());
